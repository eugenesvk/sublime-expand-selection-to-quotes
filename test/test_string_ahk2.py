...
class TestString(DeferrableViewTestCase):
  def setUp(self):
    # self.view = sublime.active_window().new_file(syntax='AutoHotkey2.sublime-syntax')
    self.view = sublime.active_window().create_output_panel(name=PACKAGE_NAME, unlisted=True)
    self.view.assign_syntax('AutoHotkey2.sublime-syntax')
  ...
  def test_all(self):
    test_set = {
      'a' : {
        'txt' :R'''"•from here ⎀ `"•" a " escape char"''',  'qb': "'", 'qe': "'", 'qp':False,
        False :   " 1              1                  "  ,
        True  :   "1                1                 "  , },
      'b' : {
        'txt' :R'''"•from here ⎀  `"" a " escape char"''',  'qb': "'", 'qe': "'", 'qp':False,
      ...
      'd' : {
        ...
        True  :   "1                1                  "  , },
      'e' : {
        'txt' :R"""'•f`b `` ⎀ `"" `'•' a "←`` acting as an escape char is skipped" """,  'qb': "'", 'qe': "'", 'qp':False,
        False :   " 1               1                                              "  ,
        True  :   "1                 1                                             "  , },
      'f' : {
        'txt' :R"""'•           "⎀  •' a '"inner double quote skipped'""",  'qb': "'", 'qe': "'", 'qp':True,
        #                       ↑ shorter ↑ but
    ...
    view = self.view
    sels = view.sel()
    # stx = sublime.find_syntax_by_name('AutoHotkey')
    # print(f"stx = {stx[0]}")
    # view.assign_syntax(stx[0].path) # fails with .scope for some reason
    # view.assign_syntax('AutoHotkey2') # fails for some reason, still uses AHK1
