...
class TestString(DeferrableViewTestCase):
  def setUp(self):
    # self.view = sublime.active_window().new_file(syntax='AutoHotkey.sublime-syntax')
    self.view = sublime.active_window().create_output_panel(name=PACKAGE_NAME, unlisted=True)
    self.view.assign_syntax('AutoHotkey.sublime-syntax')
  ...
  def test_all(self):
    test_set = {
      'a' : {
        'txt' :R'''"•from here ⎀ ""•" . a . " double quotes escaping themselves"''',  'qb': "'", 'qe': "'", 'qp':False,
        False :   " 1              1                                            "  ,
        True  :   "1                1                                           "  , },
      'b' : {
        'txt' :R'''"•from here ⎀  """ . a''',  'qb': "'", 'qe': "'", 'qp':False,
        False :   " 1              1     "  ,
        True  :   "1                1    "  , },
    }
    view = self.view
